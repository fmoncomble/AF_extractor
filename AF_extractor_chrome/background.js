...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.action === 'performExtraction') {
        try {
            ...
            selectedFormat = message.selectedFormat;
            ...
            if (
                ...
                url.includes('www.academie-francaise.fr/dire-ne-pas-dire')
            ) {
                extractDnpd(url)
                    .then((fetchedTitles) => {
                        sendResponse({
            ...
            await Promise.all(
                urls.map(async (url) => {
                    try {
                        ...
                        const bodyDivs =
                            contentDoc.querySelectorAll('.academie-columns');
                        const authorElement =
                            contentDoc.querySelector('.category.color');
                        ...
                        const authorForFileName = author
                            .replaceAll(/\s/g, '_')
                            .replaceAll(',', '');
                        ...
                        if (selectedFormat === 'xml') {
                            ...
                            fileContent = `<text author="${author}" title="${title
                                ...
                                .replaceAll(
                                    '"',
                                    '&quot;'
                                )}" date="${date}" cat="discours" sscat="${cleanPageTitle}">\n<ref target="${url}">Lien vers l'original</ref><lb></lb><lb></lb>\n${text}\n</text>`;
                        }

                        let baseFileName = `${date}_${authorForFileName}${extension}`;
                        let index = 1;

                        // Append a number to the file name to make it unique
                        while (addedFileNames.has(baseFileName)) {
                            baseFileName = `${date}_${authorForFileName}_${index}${extension}`;
                            index++;
                        }
            ...
            await Promise.all(
                Array.from(divs).map((div) => {
                    try {
                        ...
                        fileCategory = category.textContent
                            .replace('&', '&amp;')
                            .trim();
                        ...
                        if (selectedFormat === 'xml') {
                            ...
                            fileContent = `<text title="${title
                                .replaceAll('&', '&amp;')
                                .replaceAll(
                                    '"',
                                    '&quot;'
                                )}" date="${date}" cat="dnpd" sscat="${fileCategory}">\n<ref target="${itemUrl}">Lien vers l'original</ref><lb></lb><lb></lb>\n${text}\n</text>`;
                        }
